...
public class YouTubeChatPoller
{
    ...
    private async void ProcessRenderer(JsonElement renderer, HashSet<string> processedIds, Queue<string> idQueue, bool isMembership = false)
    {
        ...
        // スーパーチャット判定用
        // purchaseAmountText があれば、通貨に関係なく Super Chat
        bool isPaidMessage = false;

        // Also capture Paid message amounts.
        if (renderer.TryGetProperty("purchaseAmountText", out var purchaseAmountText) &&
            purchaseAmountText.TryGetProperty("simpleText", out var purchaseSimpleText))
        {
